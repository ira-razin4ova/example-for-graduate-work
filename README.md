...
- Кэширование frequently-запросов через Redis (TTL 10 минут)
...
| Кэширование            | Redis (Spring Data Redis)                     |
...
TTL кэша по умолчанию — 10 минут. Сериализация значений — JSON.

### Используемые кэши

| Кэш           | Ключ         | Описание                                | Очищается при                          |
|---------------|--------------|-----------------------------------------|----------------------------------------|
| `ads`         | — (all)      | Список всех объявлений                  | создании/обновлении/удалении объявления |
| `ad_details`  | ID объявления | Детали конкретного объявления           | обновлении/удалении данного объявления  |
| `user_ads`    | email        | Объявления текущего пользователя        | любых изменениях объявлений            |
| `user`        | email        | Информация профиля пользователя        | обновлении профиля/аватара             |
| `comments`    | ID объявления | Комментарии к объявлению                | добавлении/обновлении/удалении комментария |
...
│   ├── config/          # Конфигурация Spring Security, Redis (кэширование), CORS, OpenAPI
│   ├── controller/      # REST-контроллеры
│   ├── dto/             # DTO для запросов и ответов
│   ├── exception/       # Исключения, константы и глобальный обработчик
│   ├── filter/          # CORS-фильтр
│   ├── mapper/          # MapStruct-мапперы
│   ├── model/           # JPA-сущности
│   ├── repository/      # Spring Data JPA репозитории
│   ├── service/         # Бизнес-логика (с кэшированием)
