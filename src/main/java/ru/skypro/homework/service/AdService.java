 ...
 * </p>
 * <p>
 * Используется кэширование через Redis для оптимизации частых запросов:
 * </p>
 * <ul>
 *     <li>{@code ads} — список всех объявлений</li>
 *     <li>{@code ad_details} — детали конкретного объявления по ID</li>
 *     <li>{@code user_ads} — объявления текущего пользователя (ключ — email)</li>
 * </ul>
 * При создании, обновлении или удалении объявления связанные кэши автоматически очищаются.
 */
@Service
...
public class AdService {
     ...
     * <p>
     * Результат кэшируется в Redis (кэш {@code ads}) на 10 минут.
     * Кэш очищается при создании, обновлении или удалении любого объявления.
     * </p>
     *
     ...
     * </p>
     * <p>
     * После создания очищаются кэши: {@code ads} (все записи) и {@code user_ads} (данные автора).
     * </p>
     *
     ...
     * <p>
     * Результат кэшируется в Redis (кэш {@code ad_details}, ключ — ID объявления) на 10 минут.
     * Кэш очищается при обновлении или удалении данного объявления.
     * </p>
     *
     ...
     * <p>
     * При удалении очищаются все связанные кэши: {@code ads}, {@code user_ads} и {@code ad_details}.
     * </p>
     *
     * @param id          ID объявления
     * @param userDetails данные авторизованного пользователя
     ...
     * <p>
     * При обновлении очищаются все связанные кэши: {@code ads}, {@code user_ads} и {@code ad_details}.
     * </p>
     *
     * @param id          ID объявления
     ...
     * <p>
     * Результат кэшируется в Redis (кэш {@code user_ads}, ключ — email пользователя) на 10 минут.
     * Кэш очищается при создании, обновлении или удалении объявления данным пользователем,
     * а также при обновлении профиля или аватара.
     * </p>
     *
     * @param userDetails данные авторизованного пользователя
